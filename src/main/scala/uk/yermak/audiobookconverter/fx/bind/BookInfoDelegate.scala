...
class BookInfoDelegate(controller: BookInfoController) extends ConversionSubscriber {
  ...
  private[bind] def initialize(): Unit = {
    ...
    controller.title.textProperty.addListener((o: Observable) => controller.bookInfo.setTitle(controller.title.getText))
    controller.writer.textProperty.addListener((o: Observable) => controller.bookInfo.setWriter(controller.writer.getText))
    controller.narrator.textProperty.addListener((o: Observable) => controller.bookInfo.setNarrator(controller.narrator.getText))
    controller.genre.valueProperty.addListener((o: Observable) => controller.bookInfo.setGenre(controller.genre.getValue))
    controller.genre.getEditor.textProperty.addListener((o: Observable) => controller.bookInfo.setGenre(controller.genre.getEditor.getText))
    controller.series.textProperty.addListener((o: Observable) => controller.bookInfo.setSeries(controller.series.getText))
    controller.bookNo.textProperty.addListener((o: Observable) => {
      if (StringUtils.isNotBlank(controller.bookNo.getText) && StringUtils.isNumeric(controller.bookNo.getText)) controller.bookInfo.setBookNumber(controller.bookNo.getText.toInt)
    })
    controller.year.textProperty.addListener((o: Observable) => controller.bookInfo.setYear(controller.year.getText))
    controller.comment.textProperty.addListener((o: Observable) => controller.bookInfo.setComment(controller.comment.getText))
  }


  override def resetForNewConversion(conversion: Conversion) = {
    controller.bookInfo = new AudioBookInfo
    conversion.setBookInfo(controller.bookInfo)
  ...
  private[bind] def saveGenres() = {
    ...
    uniqueGenres.forEach((s: String) => sb.append(s).append("::"))
    AppProperties.setProperty("genres", sb.toString)
  }

  private[bind] def reloadGenres() = {
    ...
    if (genresProperty != null) {
      ...
      for (genre <- genres ) {
        controller.genre.getItems.addAll(genre)
      }
    }
  }


  private[bind] def copyTags(bookInfo: AudioBookInfo) = {
